...
#'
#' Summarise selected continuous variables.
#' Variables can be grouped beforehand.
#'
#' @param d A data frame
#' @param cols Columns to summarise in a vector `c()` as unquote names or character
#' @param xname [optional] <chr> Character to omit in output column names
...
#' mtcars |> group_by(vs,am) |> summ_cont(cols=c(mpg,disp))
summ_cont = function(d,cols,xname=""){
  out = d |>
    dplyr::select({{cols}}) |>
    dplyr::summarise(
      dplyr::across(
